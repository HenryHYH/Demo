...
{
    public abstract class DemoTestBase : AbpIntegratedTestBase
    {
        ...
        protected Task UsingDbContextAsync(Action<DemoDbContext> action)
        {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Action<DemoDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<DemoDbContext>())
